...
exports.getHostListPage = (req, res, next) => {
  House.find().then((regHouses) => res.render('host/host-list', { regHouses, pageTitle: "host-list" ,isLoggedIn:req.isLoggedIn,user:req.session.user}));;
};
exports.getEditHomePage = (req, res, next) => {
  ...
  House.findById(homeId).then(home => {
    if (!home) {
      ...
      res.render('host/edit-home', { home, pageTitle: "edit-home", editing,isLoggedIn:req.isLoggedIn,user:req.session.user });
    }
  })
};
